...
/**
 * Cypress E2E Test Suite: Customer Form Validation (SelectCustomer Page)
 * 
 ...
 * Total Test Cases: 20 (+ 1 optional color picker test)
 * 
 * Page URL: http://localhost:3000/selectcustomer
 * Form: "Add new customer" modal/panel
 * 
 * Note: A "Make new Color" button with hidden <input type="color"> is present on the same page 
 * but is not part of the customer creation form and is therefore excluded from this test scope.
 * 
 * Fields:
...
describe('Customer Form - Add New Customer (SelectCustomer Page)', () => {
  // Login before each test to access protected route
  beforeEach(() => {
    cy.login('admin@demo.com', '123456');
    cy.visit('/selectcustomer');
  ...
  /**
   * TC-21: Color picker button (not in form - optional test)
   * Purpose: Verify color picker button exists and works (out of form scope)
   * Note: This button is not part of the customer form but is present on the page
   */
  it('TC-21: Should open color picker dialog when "Make new Color" button is clicked', () => {
    // Verify color picker button exists on the page
    cy.contains('button', 'Make new Color').should('exist');
    
    cy.takeScreenshot('TC-21-Before-Click');
    
    // Click the color picker button
    cy.contains('button', 'Make new Color').click();
    
    cy.wait(500);
    
    cy.takeScreenshot('TC-21-After-Click');
    
    // Color picker dialog should open (browser native color picker)
    // Note: This is out of scope for form testing but documented for completeness
    cy.get('body').should('exist');
  });
});
